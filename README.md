...
php artisan vendor:publish
 Which provider or tag's files would you like to publish?:
  ...
  [1 ] Provider: SimpleCrud\Providers\SimpleCrudServiceProvider
```
...
`SimpleCrud\Services\SimpleCrudHelper` ben van / lesz pár közérdekű fg, pl. `getSimpleCrudableAppModelClasses()`
...
kell a `SimpleCrud\Contracts\CrudModelInterface` interface-t, ahol egy fg-t kell
...
**listener**, ennek implementálnia kell a `SimpleCrud\Contracts\SimpleCrudListenerInterface`. Itt lehet
...
paraméterekkl is lehet üzenni a listenernek, hogy hogy rakja össze a datatablét, vagy a `datatable` modulban
...
listenerben `SimpleCrud\Exceptions\UserCanSeeException` vagy `SimpleCrudPermissionDeniedException`-t dobunk,
